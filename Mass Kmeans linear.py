# -*- coding: utf-8 -*-
"""
Created on Mon Jul 12 15:01:18 2021
...
df = df[df['Address.Subdivision']== 'MA']
st.write(df.head())
...
fig.update_layout(
        title = 'Target location in Massachusettes USA<br>',
        geo = dict(
            scope='usa',
...
df1 = df1.values
centroids, labels, centroids_history, labels_history = k_means(df1, 5)
st.text("Displaying Centroid Values")
centroids
st.text("Displaying Labels")
labels
st.text("Dataset without Clusters Added")
df1 = pd.DataFrame(df,columns =['Address.Latitude','Address.Longitude','ID'])
st.write(df1.head())
...
fig1 = px.scatter(df1, x = 'Address.Latitude', y = 'Address.Longitude', color = 'clusters')
st.plotly_chart(fig1)

k = 5
centroids
lats = [centroids[i][0] for i in range(k)]
df1['clat'] = df1['clusters'].map(lambda x: lats[x])
longs = [centroids[i][1] for i in range(k)]
df1['clong'] = df1['clusters'].map(lambda x: longs[x])
st.write(df1.head())

st.write("Centroid Coordinates Scatter plot")
fig2 = px.scatter(df1, x = 'clat' , y = 'clong')
st.plotly_chart(fig2)

st.text("Facility Location Clusters")
sns.scatterplot(df1['Address.Latitude'],df1['Address.Longitude'], c =df1['clusters'], s = 100)
...
m2= folium.Map(location=[df1['Address.Latitude'].mean(),df1['Address.Longitude'].mean()],zoom_start=4,
             tiles='openstreetmap')
tooltip = 'Click me!'
for _, row in df1.iterrows():
    folium.Marker(
...
# need to drop the Address.name columnf
df2 = df[['Address.Latitude','Address.Longitude']]
st.write(df2.head())


m3= folium.Map(location=[df['Address.Latitude'].mean(),df['Address.Longitude'].mean()],zoom_start=4,
             tiles='openstreetmap')
for _, row in df.iterrows():
    folium.Marker(
    location=[row['Address.Latitude'],row['Address.Longitude']],
...
m5= folium.Map(location=[df1['Address.Latitude'].mean(),df1['Address.Longitude'].mean()],zoom_start=4,
             tiles='openstreetmap')
tooltip = 'Click me!'
for _, row in b.iterrows():
    folium.Marker(
